...
use crate::spaces::{BaseSpace, FiniteSpace, Space};
use std::error::Error;
use std::fmt;
...
impl AgentDef {
    ...
    where
        OS: FiniteSpace + 'static,
        AS: FiniteSpace + 'static,
    {
        match self {
...
pub struct MakeAgentError {
    agent: AgentDef,
    observation_space: Box<dyn BaseSpace>,
    action_space: Box<dyn BaseSpace>,
}
