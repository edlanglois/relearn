...
pub trait GenericSimulationHook {
    ...
    /// # Returns
    /// Whether the simulation should continue after this step.
    fn call<O, A>(&mut self, step: &FullStep<O, A>, logger: &mut dyn TimeSeriesLogger) -> bool;
}
...
impl GenericSimulationHook for StepLimit {
    ...
    fn call<O, A>(&mut self, _: &FullStep<O, A>, _: &mut dyn TimeSeriesLogger) -> bool {
...
impl GenericSimulationHook for EpisodeLimit {
    ...
    fn call<O, A>(&mut self, step: &FullStep<O, A>, _: &mut dyn TimeSeriesLogger) -> bool {
...
where
    F: FnMut(&FullStep<O, A>) -> bool,
{
    fn call(&mut self, step: &FullStep<O, A>, _: &mut dyn TimeSeriesLogger) -> bool {
        (self.f)(step)
    }
}
...
impl<O, AS: FiniteSpace> SimulationHook<O, AS::Element> for IndexedActionCounter<AS> {
    fn call(&mut self, step: &FullStep<O, AS::Element>, _: &mut dyn TimeSeriesLogger) -> bool {
...
impl GenericSimulationHook for RewardStatistics {
    fn call<O, A>(&mut self, step: &FullStep<O, A>, _logger: &mut dyn TimeSeriesLogger) -> bool {
